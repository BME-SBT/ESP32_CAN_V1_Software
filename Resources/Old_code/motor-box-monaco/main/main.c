...
void init_uart()
{
    ...
    ESP_ERROR_CHECK(uart_set_pin(UART_PORT_NUM, 1, 0, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
...
void read_motor_data(){
    while(1){
        ...
        motor_data.data[4]  = (int8_t)((power >> 8) & 0xFF);
        motor_data.data[5]  = (int8_t)(power & 0xFF);  
...
void stepMotor(int steps, bool direction) {
    ...
    for (int i = 0; i < steps; i++) {
        ...
        if(gpio_get_level(LEFT_LIM) == 0 || gpio_get_level(RIGHT_LIM) == 0) { i = steps;}
    }
}
...
void app_main(void){
    ...
    */
    ...
    while(1){
        ...
        int delay = 0;
        ESP_LOGI(TAG, "%d", controll.data[3]);
        if(controll.data[3] == 1){
            if(controll_latch == 0){
                ESP_LOGI(TAG, "calibrating");
                gpio_set_level(STEPPER_ENA, 1);
                calibrate_steering();
                controll_latch = 1;
            }
        }
        if(controll.data[4] == 1){
            if(controll_latch == 1){
                if(controll.data[3] == 0){
                    controll_latch = 0;
                    ESP_LOGI(TAG, "latch reset");

                }
                position = data_to_pos(controll.data[0]);
                mcpwm_comparator_set_compare_value(comparator, position);
            
                rotateToAngle(data_to_angle(controll.data[1]));
                ESP_LOGI(TAG, "raw %d", controll.data[1]);
                ESP_LOGI(TAG, "position %d", data_to_angle(controll.data[1])); 
            }
