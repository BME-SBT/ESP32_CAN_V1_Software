...
void app_main(void)
{
    ...
    bool ams_latch      = 1;
    bool throttle_latch = 1;
    bool steering_latch = 1;
    ...
    while(1){
        ...
        if (controll.data[0] != 127){
            throttle_latch = 1;
        } 
        if (controll.data[0] == 127){
            throttle_latch = 0;
        } 
        ai_messages.data[1] = throttle_latch;
        ...
        if(((MIN_STEERING_POSITION + MAX_STEERING_POSITION) / 2) - (STEERING_DEAD_ZONE / 2) < steering && steering < ((MIN_STEERING_POSITION + MAX_STEERING_POSITION) / 2) + (STEERING_DEAD_ZONE / 2) ){
            controll.data[1] = 127;
        }
        if (controll.data[1] != 127){
            steering_latch = 1;
        } 
        if (controll.data[1] == 127){
            steering_latch = 0;
        } 
        ai_messages.data[2] = steering_latch;

        //reading steering ena
        controll.data[3] = (gpio_get_level(STEERING_ENA) == 1) ? 0 : 1;
        

        ESP_LOGI(TAG, "Steering ena: %d", controll.data[3]);

        ams_latch = 1;
        if(steering_latch == 0 && throttle_latch == 0){
            if(buttons() == 0){
                ams_latch = 0;
            } 
        }

       while(gpio_get_level(AMS)==0 && ams_latch == 0){
        uint8_t ai_data[3] = {0, 127, 127};
        
        switch (buttons()) {
        case MODE_TAXI:
            uart_write_bytes(UART_PORT_NUM, (const char *)&taxi, sizeof(taxi));
            ESP_LOGI(TAG, "Sent state: taxi");
            break;
        
        case MODE_SLALOM:
            uart_write_bytes(UART_PORT_NUM, (const char *)&slalom, sizeof(slalom));
            ESP_LOGI(TAG, "Sent state: slalom");
            break;
        
        case MODE_DOCKING:
            uart_write_bytes(UART_PORT_NUM, (const char *)&docking, sizeof(docking));
            ESP_LOGI(TAG, "Sent state: docking");
            break;
        
        case MODE_RUNNING:
            uart_write_bytes(UART_PORT_NUM, (const char *)&running, sizeof(running));
            ESP_LOGI(TAG, "Sent state: running");
            break;

        case MODE_ERROR:
            uart_write_bytes(UART_PORT_NUM, (const char *)&error, sizeof(error));
            ESP_LOGI(TAG, "Sent state: error");
            break;
        default:
            uart_write_bytes(UART_PORT_NUM, (const char *)&ready, sizeof(ready));
            ESP_LOGI(TAG, "Sent state: ready");
            break;
        }
        ...
        case 2:
            //operation, constans green, flashing orange
            lamp(2, 1, 0);
            break;

        case 3:
            //task done, constans orange and green
            lamp(2, 2, 0);
            break;
        ...
        controll.data[0] = ai_data[1];
        controll.data[1] = ai_data[2];
        ai_messages.data[0] = ai_data[0];
       ...
       /*
        ...
        ESP_LOGI(TAG, "Sending  TAXI: %d", gpio_get_level(AI_MODE_TAXI));
        ESP_LOGI(TAG, "Sending  SLALOM: %d", gpio_get_level(AI_MODE_SLALOM));
        ESP_LOGI(TAG, "Sending  DOCKING: %d", gpio_get_level(AI_MODE_DOCKING));
        ESP_LOGI(TAG, "Sending  AI_START: %d", gpio_get_level(AI_START));

        vTaskDelay(pdMS_TO_TICKS(1000)); // debug delay
        */
